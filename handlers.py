...
async def register_handlers(dp: Dispatcher, bot_token):
        ...
        if match:
            ...
            if klad_type_forkey:
                ...
                if 'card' in active_payment_types:
                    response_message += f"💰 <i>Оплата ВРУЧНУЮ на КАРТУ (Через оператора)</i> 👉 /order_st_{klad_type_forkey}_10_5_{product_id}\n<b>{discount_text}</b>\n\n"
                    response_message += f"💰 <i>Оплата Остальные банки</i> 👉 /order_st_{klad_type_forkey}_53_5_{product_id}\n<b>{discount_text}</b>\n\n"
                    response_message += f"💰 <i>Оплата Сбербанк, Тинькофф</i> 👉 /order_st_{klad_type_forkey}_53_5_{product_id}\n<b>{discount_text}</b>\n\n"
                    keyboard.add(types.KeyboardButton(f"Через оператора order_st_{klad_type_forkey}_10_5_{product_id}"))
                    keyboard.add(
                        types.KeyboardButton(f"Оплата Остальные банки order_st_{klad_type_forkey}_53_5_{product_id}"))
                    keyboard.add(
                        types.KeyboardButton(f"Оплата Сбербанк, Тинькофф order_st_{klad_type_forkey}_53_5_{product_id}"))
    ...
    async def order_payment_method_selectio1n(message: types.Message, state: FSMContext):
        ...
        if match:
            ...
            if product_id:
                ...
                if 'btc' in active_payment_types:
                    response_message += f"💰 <i>Bitcoin (BTC)</i> 👉 /order_{klad_type_forkey}_22_5_{product_id}\n<b>{discount_text}</b>\n\n"
                    keyboard.add(types.KeyboardButton(f"Bitcoin (BTC) order_{klad_type_forkey}_22_5_{product_id}"))
    
                if 'ltc' in active_payment_types:
                    response_message += f"💰 <i>Litecoin (LTC)</i> 👉 /order_{klad_type_forkey}_24_5_{product_id}\n<b>{discount_text}</b>\n\n"
                    keyboard.add(types.KeyboardButton(f"Litecoin (LTC) order_{klad_type_forkey}_24_5_{product_id}"))
    
                response_message += f"💰 <i>Оплата с баланса</i> 👉 /order_{klad_type_forkey}_35_5_{product_id}\n<b>{discount_text}</b>\n\n"
                keyboard.add(types.KeyboardButton(f"Оплата с баланса order_{klad_type_forkey}_35_5_{product_id}"))
    
                if 'card' in active_payment_types:
                    response_message += f"💰 <i>Оплата ВРУЧНУЮ на КАРТУ (Через оператора)</i> 👉 /order_{klad_type_forkey}_10_5_{product_id}\n<b>{discount_text}</b>\n\n"
                    response_message += f"💰 <i>Оплата Сбербанк, Тинькофф</i> 👉 /order_{klad_type_forkey}_53_5_{product_id}\n<b>{discount_text}</b>\n\n"
                    response_message += f"💰 <i>Оплата Остальные банки</i> 👉 /order_{klad_type_forkey}_53_5_{product_id}\n<b>{discount_text}</b>\n\n"
                    keyboard.add(types.KeyboardButton(f"Через оператора order_{klad_type_forkey}_10_5_{product_id}"))
                    keyboard.add(
                        types.KeyboardButton(f"Оплата Сбербанк, Тинькофф order_{klad_type_forkey}_53_5_{product_id}"))
                    keyboard.add(
                        types.KeyboardButton(f"Оплата Остальные банки order_{klad_type_forkey}_53_5_{product_id}"))
        ...
        if match:
            ...
            if product_detail:
                ...
                response_message = (f"<b>💰 Вы заказали</b>\n"
                                    f"{product_name} на сумму {price} руб\n"
                                    f"в районе <b>{third_district}</b>.\n"
                                    f"До конца резерва осталось 30 минут.\n"
                                    f"Номер заказа: {order_number}.\n"
                                f"➖➖➖➖➖➖➖➖➖\n\n"
                                    f"Перешлите сообщение оператору для получения карты  @Tyssii_Oper_Ryssiii\n"
                                    f"<b>Tyssii_Oper_Ryssiii!</b>\n"
                                    f"сумму <b>{price} руб.</b>\n"
                                    f"В комментарии к платежу ничего указывать не нужно.\n"
                                    f"<b>Внимание!</b> После оплаты сообщите оператору номер заказа <b>{order_number}</b>, оплаченную сумму, дату и время платежа, а также номер кошелька, с которого Вы совершили оплату (если не терминал).\n"
    ...
    async def check_order_manual_payment(message: types.Message, state: FSMContext):
        ...
        start_time = data['start_time']
        time_passed = datetime.now() - start_time
        time_left = timedelta(minutes=30) - time_passed
    ...
    async def help_handler(message: types.Message):
        help_message = "Оператор @Tyssii_Oper_Ryssiii"
    ...
    async def manual_payment_initiation(message: types.Message, state: FSMContext):
        ...
        response_message = (
            ...
            "До конца резерва осталось 30 минут.\n"
            f"Номер заказа: {order_number}.\n\n"
            f"➖➖➖➖➖➖➖➖➖\n\n"
            f"Перешлите сообщение оператору для получения карты  @Tyssii_Oper_Ryssiii\n"
            f"<b>Tyssii_Oper_Ryssiii!</b>\n"
            f"сумму <b>{rub_amount_fee} руб.</b>\n"
            f"В комментарии к платежу ничего указывать не нужно.\n"
            f"<b>Внимание!</b> После оплаты сообщите оператору номер заказа <b>{order_number}</b>, оплаченную сумму, дату и время платежа, а также номер кошелька, с которого Вы совершили оплату (если не терминал).\n"
            f"<b>Внимание!</b> Сообщать об оплате нужно именно оператору, а не боту! Однако адрес выдаст Вам бот.\n\n"
            f"➖➖➖➖➖➖➖➖➖\n"
    ...
    async def start_order_timer2(message: types.Message, state: FSMContext, order_number: str, payment_method: str,
        ...
        for i in range(reminders):
            ...
            f"Перешлите сообщение оператору для получения карты  @Tyssii_Oper_Ryssiii\n"
            f"<b>Tyssii_Oper_Ryssiii!</b>\n"
            f"сумму <b>{data['price_rub']} руб.</b>\n"
            f"В комментарии к платежу ничего указывать не нужно.\n"
            f"<b>Внимание!</b> После оплаты сообщите оператору номер заказа <b>{order_number}</b>, оплаченную сумму, дату и время платежа, а также номер кошелька, с которого Вы совершили оплату (если не терминал).\n"
            f"<b>Внимание!</b> Сообщать об оплате нужно именно оператору, а не боту! Однако адрес выдаст Вам бот.\n\n"
            f"➖➖➖➖➖➖➖➖➖\n"
